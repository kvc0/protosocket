...
async fn generate_traffic(
    ...
    loop {
        ...
        // Alternate unary and streaming so both spawned server paths get exercised.
        let behavior = if i.is_multiple_of(2) {
        ...
        let request = Request {
            ...
            response_behavior: behavior as i32,
        };
        concurrent_count.fetch_add(1, Ordering::Relaxed);
        match behavior {
            ResponseBehavior::Unary => match client.send_unary(request) {
                Ok(completion) => {
            ...
            ResponseBehavior::Stream => match client.send_streaming(request) {
                Ok(mut completion) => {
